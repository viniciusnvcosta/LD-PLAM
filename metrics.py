...
class Metrictor:
    ...
    def table_show(resList, report, rowName="CV"):
        ...
        for i, res in enumerate(resList):
            print(
                "%6s" % (rowName + "_" + str(i + 1))
                + "".join(["%8.3f}" % res[j] for j in report])
            )
        print(
            "%6s" % ("MEAN")
            + "".join(
                ["%8.3f" % (np.mean([res[i] for res in resList])) for i in report]
            )
        )
        print("======" + "========" * len(report))
    ...
    def each_class_indictor_show(self, id2lab):
        ...
        MCCi = fill_inf(
            (TPi * TNi - FPi * FNi)
            / np.sqrt((TPi + FPi) * (TPi + FNi) * (TNi + FPi) * (TNi + FNi)),
            np.nan,
        )
        ...
        for i, c in enumerate(id2lab):
            print(
                "%30s%8.2f%8.3f%8.3f%8.3f%8.3f"
                % (c, classRate[i], MCCi[i], Pi[i], Ri[i], Fi[i])
            )
        print("-" * 70)
...
def _TPiFPiTNiFNi(classNum, Y_pre, Y):
    ...
    TPi = np.array(
        [Y_pre[:, i][Y[:, i] == 1].sum() for i in range(Y.shape[1])], dtype="float32"
    )
...
def F1(classNum, Y_pre, Y, average="micro"):
    assert average in ["micro", "macro"]
    if average == "micro":
        MiP, MiR = Precision(classNum, Y_pre, Y, average="micro"), Recall(
            classNum, Y_pre, Y, average="micro"
        )
