...
class MIMIC:
    def __init__(
        self,
        ADMISSIONSPATH="ADMISSIONS.csv",
        NOTEEVENTSPATH="NOTEEVENTS.csv",
        DIAGNOSESICDPATH="DIAGNOSES_ICD.csv",
        DICDDIAGNOSESPATH="D_ICD_DIAGNOSES.csv",
    ):
    ...
    def get_basic_data(
        ...
        out = pd.merge(
            noteevents[["HADM_ID", "TEXT"]],
            diagnosesIcd[["HADM_ID", "ICD9_CODE"]],
            how="left",
        ).dropna()
        out = (
            out.groupby(by=["HADM_ID", "TEXT"])
            .agg(lambda x: ";".join(x.values))
            .reset_index()
        )
        out = out.astype({"HADM_ID": np.int32, "TEXT": "str", "ICD9_CODE": "str"})
        out["TEXT"] = out["TEXT"].map(
            lambda x: formatParser.sub(" <br> ", contentParser.sub(" ", x.lower()))
        )
        out.to_csv(outPath)
        # get base corpus
        ...
        with open(titleOutPath, "w") as f:
            for i in tqdm(range(dIcdDiagnoses.shape[0])):
                ...
                title = (
                    title["SHORT_TITLE"].lower() + " <:> " + title["LONG_TITLE"].lower()
                )
...
class MIMIC_new:
    ...
    def get_basic_data(
        ...
        diagnosesIcd = pd.read_csv(self.DIAGNOSESICDPATH).dropna(
            subset=["HADM_ID", "ICD9_CODE"]
        )
        diagnosesIcd.ICD9_CODE = diagnosesIcd.ICD9_CODE.astype("str").map(
            lambda x: "dia_" + x
        )
        dIcdDiagnoses = pd.read_csv(self.DICDDIAGNOSESPATH)

        proceduresIcd = pd.read_csv(self.PROCEDURESICDPATH).dropna(
            subset=["HADM_ID", "ICD9_CODE"]
        )
        proceduresIcd.ICD9_CODE = proceduresIcd.ICD9_CODE.astype("str").map(
            lambda x: "pro_" + x
        )
        ...
        out = pd.merge(
            noteevents[["HADM_ID", "TEXT"]]
            .groupby("HADM_ID")
            .agg(lambda x: " <br> ".join(x.values))
            .reset_index(),
            icd[["HADM_ID", "ICD9_CODE"]],
            how="left",
        ).dropna()
        out = (
            out.groupby(by=["HADM_ID", "TEXT"])
...
class DataClass:
    def __init__(
        ...
        NOTE, ICD = [term_pattern.findall(i) for i in tqdm(data["TEXT"])], list(
            data["ICD9_CODE"].values
        )
        ...
        for i, s in tqdm(enumerate(NOTE)):
            NOTE[i] = [
                w
                if (
                    (wordCount[w] >= minCount) and (w not in stopWords) and (len(w) > 2)
                )
                else "<UNK>"
                for w in s
            ]
        ...
        for icd in self.id2icd:
            try:
                desc = (
                    (
                        icdTitles.loc[icd]["SHORT_TITLE"]
                        + " <:> "
                        + icdTitles.loc[icd]["LONG_TITLE"]
                    )
                    .lower()
                    .split()
                )
        ...
        self.tokenizedNote = np.array(
            [[self.nword2id[w] for w in n] for n in tqdm(self.notes)], dtype="int32"
        )
        self.tokenizedTitle = np.array(
            [
                [self.tword2id[w] for w in t] + [0] * (titleMaxLen - len(t))
        ...
        restIdList, testIdList = (
            train_test_split(
                range(self.totalSampleNum), test_size=testSize, random_state=seed
            )
            if testSize > 0.0
            else (list(range(self.totalSampleNum)), [])
        )
        trainIdList, validIdList = (
            train_test_split(restIdList, test_size=validSize, random_state=seed)
            if validSize > 0.0
            else (restIdList, [])
        )

        self.trainIdList, self.validIdList, self.testIdList = (
            trainIdList,
            validIdList,
            testIdList,
        )
        self.trainSampleNum, self.validSampleNum, self.testSampleNum = (
            len(self.trainIdList),
    ...
    def vectorize(
        ...
        if os.path.exists(path) and loadCache:
            ...
            corpus = (
                self.rawNOTE if noteCorpusPath is None else LineSentence(noteCorpusPath)
            )
            if method == "skipgram":
                model = Word2Vec(
                    corpus,
    ...
    def random_batch_data_stream(
        self, batchSize=128, type="train", device=torch.device("cpu")
    ):
        if type == "train":
        ...
        noteLen = np.array(
            (self.tokenizedNote == self.nword2id["<EOS>"]).argmax(axis=1) + 1,
            dtype=np.int32,
        )
        while True:
            ...
            for i in range((len(idList) + batchSize - 1) // batchSize):
                ...
                yield {
                    "noteArr": torch.tensor(
                        self.tokenizedNote[samples], dtype=torch.long, device=device
                    ),
                    "lab": torch.tensor(
                        self.Lab[samples].sum(axis=1), dtype=torch.int, device=device
                    ),
                    "noteLen": torch.tensor(noteLen[samples], dtype=torch.float32),
                }, torch.tensor(self.Lab[samples], dtype=torch.float, device=device)

    def one_epoch_batch_data_stream(
        self, batchSize=128, type="valid", device=torch.device("cpu")
    ):
        if type == "train":
    ...
    def _drop_unimportant_words(self, sents, seqMaxLen):
        ...
        for i, s in enumerate(tqdm(sents)):
            ...
            if len(sents[i]) < seqMaxLen:
                sents[i] = sents[i] + [
                    "<EOS>" for i in range(seqMaxLen - len(sents[i]))
                ]
        return sents


def redivide_dataset(dataClass, camlPath="mimic3/caml"):
    trainHID = pd.read_csv(
        os.path.join(camlPath, "train_full_hadm_ids.csv"), header=None
    )[0].values
    validHID = pd.read_csv(
        os.path.join(camlPath, "dev_full_hadm_ids.csv"), header=None
    )[0].values
    testHID = pd.read_csv(
        os.path.join(camlPath, "test_full_hadm_ids.csv"), header=None
    )[0].values
    ...
    dataClass.trainSampleNum, dataClass.validSampleNum, dataClass.testSampleNum = (
        len(trainIdList),
        len(validIdList),
        len(testIdList),
    )
...
def get_ICD_vectors(dataClass, mimicPath):
    ...
    for i, item in enumerate(tqdm(icd.itertuples())):
        ...
        inputs = tokenizer(
            tmp, padding=True, truncation=True, max_length=512, return_tensors="pt"
        )
