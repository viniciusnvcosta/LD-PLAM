...
class BaseModel:
    ...
    def train(
        ...
        optimizer = torch.optim.Adam(
            self.moduleList.parameters(), lr=lr, weight_decay=weightDecay
        )
        trainStream = dataClass.random_batch_data_stream(
            batchSize=trainSize, type="train", device=self.device
        )
        itersPerEpoch = (dataClass.trainSampleNum + trainSize - 1) // trainSize
        mtc, bestMtc, stopSteps = 0.0, 0.0, 0
        if dataClass.validSampleNum > 0:
            validStream = dataClass.random_batch_data_stream(
                batchSize=trainSize, type="valid", device=self.device
            )
        st = time.time()
        for e in range(epoch):
            for i in range(itersPerEpoch):
                ...
                if stopRounds > 0 and (e * itersPerEpoch + i + 1) % stopRounds == 0:
                    self.to_eval_mode()
                    print(
                        "After iters %d: [train] loss= %.3f;"
                        % (e * itersPerEpoch + i + 1, loss),
                        end="",
                    )
                    if dataClass.validSampleNum > 0:
                    ...
                    restNum = (
                        (itersPerEpoch - i - 1) + (epoch - e - 1) * itersPerEpoch
                    ) * trainSize
                    speed = (e * itersPerEpoch + i + 1) * trainSize / (time.time() - st)
                    print(
                        " speed: %.3lf items/s; remaining time: %.3lfs;"
                        % (speed, restNum / speed)
                    )
            if dataClass.validSampleNum > 0 and (e + 1) % saveRounds == 0:
                ...
                mtc = res[metrics]
                print("=================================")
                if (mtc > bestMtc and isHigherBetter) or (
                    mtc < bestMtc and not isHigherBetter
                ):
                    print(
                        "Bingo!!! Get a better Model with val %s: %.3f!!!"
                        % (metrics, mtc)
                    )
                    bestMtc = mtc
                    ...
                    if stopSteps >= earlyStop:
                        print(
                            "The val %s has not improved for more than %d steps in epoch %d, stop training."
                            % (metrics, earlyStop, e + 1)
                        )
                        break
        self.load("%s.pkl" % savePath, dataClass=dataClass)
        os.rename(
            "%s.pkl" % savePath, "%s_%s.pkl" % (savePath, ("%.3lf" % bestMtc)[2:])
        )
        Y_pre, Y = self.calculate_y_prob_by_iterator(
            dataClass.one_epoch_batch_data_stream(
                trainSize, type="train", device=self.device
    ...
    def save(self, path, epochs, bestMtc=None, dataClass=None):
        ...
        if dataClass is not None:
            (
                stateDict["trainIdList"],
                stateDict["validIdList"],
                stateDict["testIdList"],
            ) = (dataClass.trainIdList, dataClass.validIdList, dataClass.testIdList)
            stateDict["nword2id"], stateDict["tword2id"] = (
                dataClass.nword2id,
                dataClass.tword2id,
            )
            stateDict["id2nword"], stateDict["id2tword"] = (
                dataClass.id2nword,
                dataClass.id2tword,
            )
            stateDict["icd2id"], stateDict["id2icd"] = (
                dataClass.id2icd,
                dataClass.icd2id,
            )
    ...
    def load(self, path, map_location=None, dataClass=None):
        ...
        if dataClass is not None:
            ...
            dataClass.nword2id, dataClass.tword2id = (
                parameters["nword2id"],
                parameters["tword2id"],
            )
            dataClass.id2nword, dataClass.id2tword = (
                parameters["id2nword"],
                parameters["id2tword"],
            )
            dataClass.id2icd, dataClass.icd2id = (
                parameters["icd2id"],
                parameters["id2icd"],
            )
        print(
            "%d epochs and %.3lf val Score 's model load finished."
    ...
    def calculate_y_prob_by_iterator(self, dataStream):
        ...
        YArr, Y_preArr = np.vstack(YArr).astype("int32"), np.vstack(Y_preArr).astype(
            "float32"
        )
...
class LD_PLAM(BaseModel):
    def __init__(
        ...
        self.moduleList = nn.ModuleList(
            [self.embedding, self.biLSTM, self.LNandDP, self.icdAttn, self.fcLinear]
        )
        self.crition = (
            nn.MultiLabelSoftMarginLoss()
            if not useCircleLoss
            else MultiLabelCircleLoss()
        )
    ...
    def calculate_y_logit(self, input):
        ...
        if torch.cuda.device_count() > 1:
            ...
            x = nn.parallel.data_parallel(self.fcLinear, x).squeeze(
                dim=2
            )  # => batchSize × classNum
        else:
...
class KAICD(BaseModel):
    def __init__(
        ...
        self.titles, self.titleLens = torch.tensor(
            tokenizedTitle, dtype=torch.long, device=device
        ), torch.tensor(titleLen, dtype=torch.long, device=device)
        self.noteEmbedding = TextEmbedding(
            noteEmbedding, freeze=False, dropout=embDropout, name="noteEmbedding"
        ).to(device)
        self.titleEmbedding = TextEmbedding(
            titleEmbedding, freeze=False, dropout=embDropout, name="titleEmbedding"
        ).to(device)
        self.textCNN = TextCNN(noteFeaSize, filterSize, contextSizeList).to(device)
        self.textBiRNN = TextGRU(titleFeaSize, hiddenSize, bidirectional=True).to(
            device
        )
        self.simpleAttn = SimpleAttention(hiddenSize * 2).to(device)
        self.knowledgeAttn = KnowledgeAttention(
            len(contextSizeList) * filterSize, hiddenSize * 2
        ).to(device)
        self.fcLinear = MLP(
            len(contextSizeList) * filterSize + hiddenSize * 2,
            classNum,
    ...
    def calculate_y_logit(self, input):
        ...
        noteConved = self.textCNN(
            self.noteEmbedding(x)
        )  # => batchSize × scaleNum*filterSize
        labelEncoded = self.textBiRNN(
            self.titleEmbedding(self.titles)
        )  # => labelNum × seqLen × hiddenSize*2
        labelEncoded = self.simpleAttn(labelEncoded)  # => labelNum × hiddenSize*2

        knowledgeAttned = self.knowledgeAttn(
            noteConved, labelEncoded
        )  # => batchSize × hiddenSize*2
        return {
            "y_logit": self.fcLinear(torch.cat([knowledgeAttned, noteConved], dim=1))
        }  # => batchSize × classNum
...
class CAML(BaseModel):
    def __init__(
        ...
        self.textCNN = TextCNN(
            noteEmbedding.shape[1], filterSize, contextSizeList, reduction="none"
        ).to(device)
        self.ICDAttn = ICDAttention(filterSize * len(contextSizeList), classNum).to(
            device
        )
        self.fcLinear = MLP(filterSize * len(contextSizeList), classNum).to(device)
        self.moduleList = nn.ModuleList(
            [self.noteEmbedding, self.textCNN, self.ICDAttn, self.fcLinear]
        )
        if self.DR:
            self.titleEmbedding = TextEmbedding(
                titleEmbedding, freeze=False, dropout=embDropout, name="titleEmbedding"
            ...
            self.labConv = TextCNN(
                titleEmbedding.shape[1], filterSize, contextSizeList, name="labConv"
            ).to(device)
    ...
    def calculate_y_logit(self, input):
        ...
        if self.DR:
            ...
            dr = ((z - self.fcLinear.out.weight) ** 2).sum(
                dim=1
            )  # => batchSize × classNum
            dr = (dr * input["lab"]).sum(axis=1) / (
                input["lab"].sum(axis=1) + 1
            )  # => batchSize
...
class MVCLDA(BaseModel):
    def __init__(
        ...
        self.DR = DR
        self.noteEmbedding = TextEmbedding(
            noteEmbedding, freeze=False, dropout=embDropout
        ).to(device)
        self.textCNN = TextCNN(
            noteEmbedding.shape[1], filterSize, contextSizeList, reduction="cpool"
        ).to(device)
        self.ICDAttn = ICDAttention(filterSize, classNum).to(device)
        self.fcLinear = MLP(filterSize, classNum).to(device)
        self.kLinear = MLP(
            1, classNum, actFunc=nn.Sigmoid, outAct=True, name="kLinear"
        ).to(device)
        self.moduleList = nn.ModuleList(
            [
                self.noteEmbedding,
    ...
    def calculate_y_logit(self, input):
        ...
        y = (
            self.fcLinear.out.weight.mul(x).sum(dim=2)
            + self.fcLinear.out.bias
            + self.kLinear(l)
        )
        if self.DR:
            z, _ = torch.max(
                self.labConv(self.titleEmbedding(self.titles)), dim=1
            )  # => classNum × feaSize
