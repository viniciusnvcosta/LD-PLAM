...
class TextEmbedding_old(nn.Module):
    def __init__(self, embeding, freeze=False, dropout=0.2, name="textEmbedding"):
        ...
        self.embedding = nn.Embedding.from_pretrained(
            torch.tensor(embeding, dtype=torch.float32), freeze=freeze
        )
        self.dropout = nn.Dropout(p=dropout)
        self.name = name
...
class TextEmbedding(nn.Module):
    def __init__(self, embedding, dropout=0.3, freeze=False, name="textEmbedding"):
        ...
        self.name = name
        self.embedding = nn.Embedding.from_pretrained(
            torch.tensor(embedding, dtype=torch.float32), freeze=freeze
        )
...
class ResDilaCNNBlocks(nn.Module):
    def __init__(
        ...
        for i in range(blockNum):
            self.blockLayers.add_module(
                f"ResDilaCNNBlock{i}",
                ResDilaCNNBlock(
                    dilaSizeList[i % len(dilaSizeList)], filterSize, dropout=dropout
                ),
            )
        self.name = name
    ...
    def forward(self, x):
        ...
        x = self.blockLayers(x.transpose(1, 2)).transpose(
            1, 2
        )  # => batchSize × seqLen × filterSize
...
class TextCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = [
            conv(x).transpose(1, 2) for conv in self.conv1dList
        ]  # => scaleNum * (batchSize × seqLen × filterSize)
...
class TextLSTM(nn.Module):
    ...
    def forward(self, x, xlen=None):
        ...
        output, hn = self.biLSTM(
            x
        )  # output: batchSize × seqLen × hiddenSize*2; hn: numLayers*2 × batchSize × hiddenSize
        if xlen is not None:
    ...
    def __init__(
        ...
        if not compress:
            ...
            self.decode = nn.Sequential(
                nn.Linear(attnList[-1], classNum), nn.BatchNorm1d(inSize), nn.ReLU()
            )
        for os in attnList:
...
class DeepICDDescAttention(nn.Module):
    def __init__(
        ...
        self.dropout = nn.Dropout(p=hdnDropout)
        self.labDescVec = (
            nn.Parameter(torch.tensor(labDescVec, dtype=torch.float32))
            if labDescVec is not None
            else None
        )
        self.name = name
    ...
    def forward(self, X, labDescVec=None):
        ...
        alpha = F.softmax(
            torch.matmul(X, labDescVec.transpose(0, 1)), dim=1
        )  # => batchSize × os × classNum
...
class LuongAttention(nn.Module):
    ...
    def dot_score(self, hidden, encoderOutput):
        ...
        return torch.matmul(
            encoderOutput, hidden.transpose(-1, -2)
        )  # => batchSize × seq_len × classNum
...
class SimpleAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        return self.actFunc(
            torch.matmul(input.transpose(1, 2), alpha).squeeze(2)
        )  # => batchSize × inSize
...
class KnowledgeAttention(nn.Module):
    def __init__(self, noteFeaSize, titleFeaSize, name="knowledgeAttention"):
        ...
        self.linear = nn.Sequential(nn.Linear(noteFeaSize, titleFeaSize), nn.Tanh())
        self.labWeight = None
        self.name = name
    ...
    def forward(self, noteConved, titleEncoded):
        ...
        attnWeight = F.softmax(
            torch.matmul(x, titleEncoded.transpose(0, 1)), dim=1
        )  # => batchSize × titleNum
...
class InterationAttention(nn.Module):
    def __init__(
        self, feaSize1, feaSize2, dropout=0.0, attnType="poolAttn", name="interAttn"
    ):
        super(InterationAttention, self).__init__()
        self.attnFunc = {
            "poolAttn": self.pooling_attention,
    ...
    def pooling_attention_s(self, x, y):
        ...
        x = torch.matmul(x.transpose(1, 2), F.softmax(xAlpha, dim=1)).squeeze(
            dim=2
        )  # => batchSize × feaSize1
        yAlpha, _ = torch.max(alpha, dim=1, keepdim=True)  # => batchSize × 1 × seqLen2
        y = torch.matmul(F.softmax(yAlpha, dim=2), y).squeeze(
            dim=1
        )  # => batchSize × feaSize2
    ...
    def concat_simple_attention(self, x, y):
        ...
        u = torch.cat(
            [x, y_pooled.unsqueeze(dim=1).expand(-1, x.shape[1], -1)], dim=2
        )  # => batchSize × seqLen1 × (feaSize1+feaSize2)
        v = torch.cat(
            [y, x_pooled.unsqueeze(dim=1).expand(-1, y.shape[1], -1)], dim=2
        )  # => batchSize × seqLen2 × (feaSize1+feaSize2)
        x, y = (
            self.simpleAttn1(u)[:, : self.feaSize1],
            self.simpleAttn2(v)[:, : self.feaSize2],
        )
        return torch.cat([x, y], dim=1)  # => batchSize × (feaSize1+feaSize2)
    ...
    def plane_attention_s(self, x, y):
        ...
        v = y.unsqueeze(dim=1)  # => batchSize × 1 × seqLen2 × feaSize2
        alpha = torch.sum(u * v, dim=3)  # => batchSize × seqLen1 × seqLen2
        alpha = F.softmax(alpha.flatten(1, 2), dim=1).unsqueeze(
            dim=1
        )  # => batchSize × 1 × seqLen1*seqLen2

        x, y = x.unsqueeze(dim=2).expand(-1, -1, y.shape[1], -1), y.unsqueeze(
            dim=1
        ).expand(
            -1, x.shape[1], -1, -1
        )  # => batchSize × seqLen1 × seqLen2 × feaSize
        xy = torch.cat([x, y], dim=3).flatten(
            1, 2
        )  # => batchSize × seqLen1*seqLen2 × (feaSize1+feaSize2)
        return torch.matmul(alpha, xy).squeeze(
            dim=1
        )  # => batchSize × (feaSize1+feaSize2)
    ...
    def forward(self, x, y):
        ...
        return self.dropout(
            self.attnFunc[self.attnType](x, y)
        )  # => batchSize × (feaSize1+feaSize2)
...
class SelfAttention(nn.Module):
    def __init__(self, featureSize, dk, multiNum, name="selfAttn"):
        ...
        self.WQ = nn.ModuleList(
            [nn.Linear(featureSize, self.dk) for i in range(multiNum)]
        )
    ...
    def forward(self, x, xlen=None):
        # x: batchSize × seqLen × feaSize; xlen: batchSize
        queries = [
            self.WQ[i](x) for i in range(self.multiNum)
        ]  # => multiNum*(batchSize × seqLen × dk)
        keys = [
            self.WK[i](x) for i in range(self.multiNum)
        ]  # => multiNum*(batchSize × seqLen × dk)
        values = [
            self.WV[i](x) for i in range(self.multiNum)
        ]  # => multiNum*(batchSize × seqLen × dk)
        scores = [
            torch.bmm(queries[i], keys[i].transpose(1, 2)) / np.sqrt(self.dk)
            for i in range(self.multiNum)
        ]  # => multiNum*(batchSize × seqLen × seqLen)
        # mask <EOS> padding
        if xlen is not None:
            for i in range(len(scores)):
                mask = torch.zeros(
                    scores[0].shape, dtype=torch.float32, device=scores[i].device
                )  # => batchSize × seqLen × seqLen
                for j, k in enumerate(xlen):
                    mask[j, :, k - 1 :] -= 999999
                scores[i] = scores[i] + mask
        z = [
            torch.bmm(F.softmax(scores[i], dim=2), values[i])
            for i in range(self.multiNum)
        ]  # => multiNum*(batchSize × seqLen × dk)
...
class SimpleSelfAttention(nn.Module):
    ...
    def forward(self, x, xlen=None):
        ...
        score = torch.bmm(querie, key.transpose(1, 2)) / np.sqrt(
            self.feaSize
        )  # => batchSize × seqLen × seqLen
        # mask <EOS> padding
        if xlen is not None:
            mask = torch.zeros(
                score.shape, dtype=torch.float32, device=score.device
            )  # => batchSize × seqLen × seqLen
            for j, k in enumerate(xlen):
                mask[j, :, k - 1 :] -= 999999
            score = score + mask
...
class FFN(nn.Module):
    ...
    def forward(self, x, z):
        ...
        return z + self.dropout(
            self.layerNorm2(ffnx)
        )  # => batchSize × seqLen × feaSize
...
class TextTransformer(nn.Module):
    def __init__(
        ...
        posEmb = [
            [
                np.sin(pos / 10000 ** (2 * i / featureSize))
                if i % 2 == 0
                else np.cos(pos / 10000 ** (2 * i / featureSize))
                for i in range(featureSize)
            ]
            for pos in range(seqMaxLen)
        ]
        self.posEmb = nn.Parameter(
            torch.tensor(posEmb, dtype=torch.float32), requires_grad=False
        )  # seqLen × feaSize
        self.transformerLayers = nn.Sequential(
            OrderedDict(
                [
                    (
                        "transformer%d" % i,
                        Transformer(featureSize, dk, multiNum, dropout),
                    )
...
class Transformer_Wcnn(nn.Module):
    ...
    def forward(self, x):
        ...
        z = [
            torch.bmm(F.softmax(score[i], dim=2), values[i])
            for i in range(self.multiNum)
        ]  # => multiNum*(batchSize × seqLen × dk)
        z = self.WO(torch.cat(z, dim=2))  # => batchSize × seqLen × feaSize
        z = x + self.dropout(self.layerNorm1(z))  # => batchSize × seqLen × feaSize
        ...
        self.transformerLayers = nn.Sequential(
            OrderedDict(
                [
                    (
                        "transformer%d" % i,
                        Transformer_Wcnn(featureSize, dk, multiNum, seqMaxLen, dropout),
                    )
                    for i in range(layersNum)
                ]
            )
...
class HierarchicalSoftmax(nn.Module):
    ...
    def forward(self, x):
        ...
        y += [F.softmax(linear(x), dim=1) for linear in self.linearList[1:]]
        y = torch.cat(
            [y[0][:, i - 1].unsqueeze(1) * y[i] for i in range(1, len(y))], dim=1
        )  # => batchSize × classNum
...
class MultiLabelCircleLoss(nn.Module):
    ...
    def forward(self, Y_logit, Y):
        ...
        for yp, yt in zip(Y_logit, Y):
            neg = yp[yt == 0]
            pos = yp[yt == 1]
            loss += torch.log(1 + torch.exp(neg).sum()) + torch.log(
                1 + torch.exp(-pos).sum()
            )
...
class MultiTaskCEL(nn.Module):
    def __init__(self, lossBalanced=True, ageW=1, genderW=1, name="MTCEL"):
        ...
        self.genderCriterion, self.ageCriterion = (
            nn.CrossEntropyLoss(),
            nn.CrossEntropyLoss(),
        )  # ContinusCrossEntropyLoss()#
        self.genderS, self.ageS = nn.Parameter(
            torch.zeros(1, dtype=torch.float), requires_grad=lossBalanced
        ), nn.Parameter(torch.zeros(1, dtype=torch.float), requires_grad=lossBalanced)
    ...
    def forward(self, genderY_logit, genderY, ageY_logit, ageY):
        if self.lossBalanced:
            return (
                self.genderW
                * torch.exp(-self.genderS)
                * self.genderCriterion(genderY_logit, genderY)
                + self.ageW
                * torch.exp(-self.ageS)
                * self.ageCriterion(ageY_logit, ageY)
                + (self.genderS + self.ageS) / 2
            )
        else:
            return self.genderW * self.genderCriterion(
                genderY_logit, genderY
            ) + self.ageW * self.ageCriterion(ageY_logit, ageY)
